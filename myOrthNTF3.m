function [Q, alpha] = myOrthNTF3(layers_L,num_N, num_V, num_C, B_init,  F_init, G_init, p, beta, maxIter,NMF_iter)
...
while(Isconverg == 0)  
    ...
    M_tensor = cat(3, M{:,:});
    M_vector = M_tensor(:);
    [myj, ~] = wshrinkObj_weight_lp(M_vector, beta*betaf./rho, sX, 0, 3, p);
    ...
    % if ismember(iter, [0, 10, 60,70, 90,120,150])
    %     % 使用提供的方法计算F_final
    %     F_sum = Q{1} ;
    %     for v = 2:num_V
    %         F_sum = F_sum + Q{v} ;
    ...
    %     % [~, Y_pre] = max(F_final, [], 2); 
    %     % 应用t-SNE进行降维
    %     mappedX = tsne(real(F_final),'Algorithm','exact','Distance','cosine');
    % 
    %     % 创建图形进行可视化
    %     figure;
